...
def compare_lines(source, target):
    source_tokens = [token.text for token in list(nlp(str(source)))]
    target_tokens = [token.text for token in list(nlp(str(target)))]
...
def process_chunk(input_chunk):
    chunk_processed_lines = 0
    chunk_corrected_lines = 0
    
    chunk_output_source_lines = []
    chunk_output_target_lines = []
    for source, target in input_chunk:
        if compare_lines(source, target) == 1:
            chunk_output_source_lines.append(source)
            chunk_output_target_lines.append(target)
            chunk_corrected_lines += 1
        
        chunk_processed_lines += 1
    return {
        "chunk_output_source_lines": chunk_output_source_lines,
        "chunk_output_target_lines": chunk_output_target_lines,
        "chunk_processed_lines": chunk_processed_lines,
        "chunk_corrected_lines": chunk_corrected_lines
    }
...
def select_lines_with_changes(input_source, input_target, 
    ...
    input_lines = list(zip(input_source_lines, input_target_lines))
    

    
    #chunks_input_lines = np.array_split(input_lines, count_chunks)
    ...
    for chunk_num in range(count_chunks):
        ...
        for res in all_result_maps:
            output_source_lines.extend(res["chunk_output_source_lines"])
            output_target_lines.extend(res["chunk_output_target_lines"])
            processed_lines += res["chunk_processed_lines"]
            corrected_lines += res["chunk_corrected_lines"]
